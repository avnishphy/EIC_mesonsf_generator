...
For scattered particles (scat_e, pion, neutron):
  - pT, pz, p (total momentum)
  - theta, phi in radians
  - pseudorapidity (eta)

Usage example:
  python analyze_kinematics.py --input-file k_neutron_crossing_0_10.0on100.0.root --energy 10x100 --max-events 100000
"""
...
branches = ["P_Inc.", "e_Inc.", "e_Scat.", "pi.", "n_scat."]

# Friendly particle names for everything else
particles = ["inc_p", "inc_e", "scat_e", "pion", "neutron"]
...
scattered_particles = ["scat_e", "pion", "neutron"]
...
def create_histograms(e_energy, p_energy):
    ...
    # Scattered pion histograms
    # pion momentum can be substantial fraction of beam energy
    max_p = max(e_energy, p_energy) * 0.8
    hists[("pion", "pt")] = Hist(RegAx(100, 0, p_max*0.5, name="pt", label="pT [GeV/c]"))
    hists[("pion", "pz")] = Hist(RegAx(200, -p_max*0.5, p_max, name="pz", label="pz [GeV/c]"))
    hists[("pion", "p")] = Hist(RegAx(200, 0, p_max, name="p", label="P [GeV/c]"))
    hists[("pion", "theta")] = Hist(RegAx(180, 0, np.pi, name="theta", label=r"$\theta$ [rad]"))
    hists[("pion", "phi")] = Hist(RegAx(360, -np.pi, np.pi, name="phi", label=r"$\phi$ [rad]"))
    hists[("pion", "eta")] = Hist(RegAx(100, -5, 5, name="eta", label=r"$\eta$ (pseudorapidity)"))
    
    # Scattered neutron histograms
    # neutron will carry remaining momentum
    hists[("neutron", "pt")] = Hist(RegAx(100, 0, 1, name="pt", label="pT [GeV/c]"))
    hists[("neutron", "px")] = Hist(RegAx(200, -p_energy*0.03, p_energy*0.03, name="px", label="px [GeV/c]"))
    hists[("neutron", "py")] = Hist(RegAx(200, -p_energy*0.03, p_energy*0.03, name="py", label="py [GeV/c]"))
    hists[("neutron", "pz")] = Hist(RegAx(200, p_max*0.7, p_max*1.05, name="pz", label="pz [GeV/c]"))
    hists[("neutron", "p")] = Hist(RegAx(200, p_max*0.7, p_max*1.05, name="p", label="P [GeV/c]"))
    hists[("neutron", "angle_z_mrad")] = Hist(RegAx(100, 0, 10, name="angle_z", label="Min. angle with Z [mrad]"))
...
def fill_histograms(hists, chunk):
    ...
    for particle in particles:
        ...
        if particle in incident_particles + ["neutron"]:
