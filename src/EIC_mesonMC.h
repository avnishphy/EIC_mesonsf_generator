...
// const double pSMax=  1.47; // spectator particle energy radius in the proton rest frame. calculated for keeping -t<1
...
const double minus_t_max = 2;

const double pSMax = sqrt(pow(((MProton*MProton + MNeutron*MNeutron + minus_t_max)/(2*MProton)),2) - MNeutron*MNeutron);// spectator particle momentum radius in the proton rest frame. calculated for keeping -t<1
// the "1" can be replaced by the value of maximum -t that we want.
...
const double CrossingTheta = 0.0; // zero crossing; afterburner includes the crossing effects
