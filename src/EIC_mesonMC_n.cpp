...
int mainx(double xMin,double xMax, double Q2Min,double Q2Max, double rnum, const int nevts, const double pbeam, const double kbeam, bool smear){
  ...
  double pi_momx_lab,pi_momy_lab,pi_momz_lab,pi_energy_lab ;
  ...
  double n_momx_lab,n_momy_lab,n_momz_lab,n_energy_lab ;
  ...
  sprintf(rName,"/w/eic-scshelf2104/users/singhav/EIC_mesonsf_generator/OUTPUTS/pion_neutron_v2/pion_neutron_crossing_%.3f-%.1fon%.1f_x%.4f-%.4f_q%.1f-%.1f.root", CrossingTheta, kBeam, PBeam,xMin,xMax,Q2Min,Q2Max);
  ...
  double xsec_taggedLN_dis;
  double f2N;   
  Double_t qMag, pDotq;
	
  double Q2, xbj, mandelstem_t, TDIS_znq, missing_mass_squared, y_inelasticity;
  ...
  proctree->Branch("mandelstem_t", &mandelstem_t, "mandelstem_t/D");
  proctree->Branch("TDIS_znq", &TDIS_znq, "TDIS_znq/D");
  proctree->Branch("missing_mass_squared", &missing_mass_squared, "missing_mass_squared/D");
  ...
  proctree->Branch("xsec_taggedLN_dis", &xsec_taggedLN_dis, "xsec_taggedLN_dis/D");
  ...
  // TDIS detected pion information
  proctree->Branch("pi_momx_lab", &pi_momx_lab, "pi_momx_lab/D");
  proctree->Branch("pi_momy_lab", &pi_momy_lab, "pi_momy_lab/D");
  proctree->Branch("pi_momz_lab", &pi_momz_lab, "pi_momz_lab/D");
  proctree->Branch("pi_energy_lab", &pi_energy_lab, "pi_energy_lab/D");

  // TDIS spectator neutron information
  proctree->Branch("n_momx_lab", &n_momx_lab, "n_momx_lab/D");
  proctree->Branch("n_momy_lab", &n_momy_lab, "n_momy_lab/D");
  proctree->Branch("n_momz_lab", &n_momz_lab, "n_momz_lab/D");
  proctree->Branch("n_energy_lab", &n_energy_lab, "n_energy_lab/D");
  ...
  ofstream OUT_lund (Form("/w/eic-scshelf2104/users/singhav/EIC_mesonsf_generator/OUTPUTS/pion_neutron_v2/pion_neutron_crossing_%.3f_%.1fon%.1f_x%.4f-%.4f_q%.1f-%.1f_lund.dat", CrossingTheta, kBeam, PBeam,xMin,xMax,Q2Min,Q2Max), ios::trunc);
  ofstream OUT_pythia (Form("/w/eic-scshelf2104/users/singhav/EIC_mesonsf_generator/OUTPUTS/pion_neutron_v2/pion_neutron_crossing_%.3f_%.1fon%.1f_x%.4f-%.4f_q%.1f-%.1f_pythia.dat", CrossingTheta, kBeam, PBeam,xMin,xMax,Q2Min,Q2Max), ios::trunc);
  ofstream OUT_hepmc (Form("/w/eic-scshelf2104/users/singhav/EIC_mesonsf_generator/OUTPUTS/pion_neutron_v2/pion_neutron_crossing_%.3f_%.1fon%.1f_x%.4f-%.4f_q%.1f-%.1f_hepmc.dat", CrossingTheta, kBeam, PBeam,xMin,xMax,Q2Min,Q2Max), ios::trunc);
  ...
  OUT_lund << setiosflags(ios::left)  << setiosflags(ios::fixed)  << "============================================" << endl;
  OUT_lund << setiosflags(ios::left)  << setiosflags(ios::fixed)  <<"                 "  <<  "Number of Particles" << " \t "  << "xBj" << " \t " << "Q2"  << " \t " << "s_e"  << " \t " << "1.0" << " \t " << "xpi" << " \t" << "ypi" << " \t"  << "tpi"  << " \t"  <<  " \t" << "xsec_inclusive_dis" << " \t" << "xsec_taggedLN_dis" << endl;
  ...
  while(MEvts<NEvts+1){
    ...
    // Progress bar
    // if(iEvt%1000==0) {	    
    // Progress bar changed iEvts to MEvts for while loop
    if (MEvts%1000==0){
      int barWidth = 70;
    ...
    */
    ...
    missing_mass_squared = (qVirtual_Rest + pSpectator_Rest).Mag2();
    ...
    */
    ...
    // if (TMath::Sqrt(missing_mass_squared) > PBeam/2){
    //   cout << "---->TDIS missing mass =" << TMath::Sqrt(missing_mass_squared)  << endl;
    // }
    ...
    n_momx_lab = pScatterNeutron_Vertex.X();
    n_momy_lab = pScatterNeutron_Vertex.Y();
    n_momz_lab = pScatterNeutron_Vertex.Z();
    n_energy_lab = sqrt(n_momx_lab*n_momx_lab+n_momy_lab*n_momy_lab+n_momz_lab*n_momz_lab+MProton*MProton);
      
    vnx_Lab = 0.0;
    vny_Lab = 0.0;
    vnz_Lab = 0.0;
    
    pn_Lab = sqrt(n_momx_lab*n_momx_lab+n_momy_lab*n_momy_lab+n_momz_lab*n_momz_lab);
    ...
    pi_momx_lab = pScatterPion_Vertex.X();
    pi_momy_lab = pScatterPion_Vertex.Y();
    pi_momz_lab = pScatterPion_Vertex.Z();
    pi_energy_lab = sqrt(pi_momx_lab*pi_momx_lab+pi_momy_lab*pi_momy_lab+pi_momz_lab*pi_momz_lab+mPion*mPion);
    ...
    // for debugging purpose
    // cout  << "(7L) pion Vertex, px= " << pi_momx_lab << ", py= " << pi_momy_lab <<
    // ", pz= " << pi_momz_lab << ", Epi= " << pi_energy_lab << endl;
    ...
    */
	...
	FLAG = combination of splitting function/PDF (0:\pi, 1:\rho, 2:\pi\Delta)
      */
      //  typ = 1 ! s-dependent expon FORM FACTOR (L=1.56,dis=1,FLAG=0)      
      if (typ == 1){
      fpi = fpifac*f2pi(P_pi, xbj, theta_p2/D2R);
      }
      
      //  typ = 2 ! COV DIP FORM FACTOR      
      if (typ == 2){
      fpi = fpifac*f2picov(P_pi, xbj, theta_p2/D2R);
      }
      
      //  typ = 3 ! t-dependent expon FORM FACTOR (L=0.85,dis=1,FLAG=0)      
      if (typ == 3){
      fpi = fpifac*f2pitexp(P_pi, xbj,theta_p2/D2R);
      }
      //  typ = 4 ! Paul-Villas FORM FACTOR (L=0.27,dis=1,FLAG=0)      
      if (typ == 4){
      fpi = fpifac*f2pipaulvillas(P_pi, xbj, theta_p2/D2R);
      }
      
      //  typ = 5 ! ZEUS parameterization with F2N (proton SF)      
      if (typ == 5){
	    fpi = fpifac*f2piZEUS(xbj,invts.Q2,inucl);
      }
      ...
      // Testing of "y" variable: assumption: same kinematic variables:  x=xbj, k=P_pi, th=theta_p2
      ...
      if(fpi>0){
      cout << "***************       ******************      *********************" << endl;
      //cout << "test outout GRV xVpi = " << xVpiR << " , at xbj= "<< xbj <<", Q2= " << invts.Q2 << endl;
      //cout << "test outout GRV xSpi = " << xSpiR << " , at xbj= "<< xbj <<", Q2= " << invts.Q2 << endl;
    ...
    /*
      ...
      << P_pi << ", xbj= "<< xbj << ", theta_p2(deg)= "<< theta_p2 << endl; 
    */

    invts.tSpectator = MIon*MIon+MSpectator*MSpectator - 2.*pSpectator_Vertex.Dot(PIncident_Vertex);
    mandelstem_t = invts.tSpectator;
    
    tmin = ((1-xL)/xL)*(MSpectator*MSpectator-xL*MIon*MIon);// tmin is the minimumn kinematically allowed value of |t| for a given xL and thus positive

    // mandelstem_t = -((xL*p_energy_incident*pScatterNeutron_Vertex.Theta()*xL*p_energy_incident*pScatterNeutron_Vertex.Theta())/xL)-tmin;
    // mandelstem_t = (-1*pow((xL*p_energy_incident*pScatterNeutron_Vertex.Theta()),2)/xL) - tmin;

    if (mandelstem_t < tmin && mandelstem_t > -tmin){
      continue;
    }

    // cout << "---->t compare (invts vs TDIS) =" << invts.tSpectator << " " << mandelstem_t << endl;
    // invts.tPrime     = 2.*pSpectator_Vertex.Dot(PIncident_Vertex) - MIon*MIon; // invts.tPrime = -MSpectator??
    invts.tPrime     = (-1*invts.tSpectator) - tmin; // absolute difference between t and tmin. Remember that t is -ve thus, -t is +ve. tmin is +ve by definition.
    p_ST = pSpectator_Vertex -(pDotq/qMag/qMag)*qVirtual_Rest;		
    ...
    if (xbj > 0.001 && xbj < 1.0){
      ...
      // xsec_inclusive_dis    = cdissigma_p(xbj,lalasticity,invts.Q2,invts.nu,kScattered_Rest.E());
      xsec_taggedLN_dis   = xsec_inclusive_dis * (fpi/f2N); 
    }
    ...
    /*
      ...
      cout << "(10) " <<"xsec_inclusive_dis=  " << xsec_inclusive_dis  <<  ", xsec_taggedLN_dis= " << xsec_taggedLN_dis
      << ",  f2N= " <<  f2N << endl;
    ...
    */
     ...
     * **********************************************************************************/
    /*
    OUT_lund << setiosflags(ios::left)  << setiosflags(ios::fixed)  <<"                 "  <<  NumPtls << " \t " <<  scientific  << invts.xBj << " \t " << invts.Q2  << " \t " << invts.s_e  << " \t " << "1.0" << " \t " << xpi << " \t" << ypi << " \t"  << tpi  << " \t"  <<  " \t" << xsec_inclusive_dis << " \t" << xsec_taggedLN_dis << endl;
    ...
    // final state pion
    OUT_lund << setiosflags(ios::left) << setiosflags(ios::fixed) <<  "\t" << "4" << " \t " << pi_particle_charge << " \t " << "1" << " \t " << pi_particle_id << " \t " << "0" <<  " \t "<< "1" <<  " \t "<< scientific << pi_momx_lab << " \t " << pi_momy_lab << " \t " << pi_momz_lab << " \t " << pi_energy_lab << " \t " << pimass << " \t " << vpix_Lab  << " \t " << vpiy_Lab << " \t " << vpiz_Lab << endl; 
    // final state neutron (TDIS)
    OUT_lund << setiosflags(ios::left) << setiosflags(ios::fixed) <<  "\t" << "5" << " \t " << sp_particle_charge << " \t " << "1" << " \t " << sp_particle_id << " \t " << "0" <<  " \t "<< "1" <<  " \t "<< scientific << n_momx_lab << " \t " << n_momy_lab << " \t " << n_momz_lab << " \t " << n_energy_lab << " \t " << spmass << " \t " << vnx_Lab  << " \t " << vny_Lab << " \t " << vnz_Lab << endl;  
    */
    /************************************************************************************
     * 
     * DD4Hep input
    ...
    OUT_pythia << setiosflags(ios::left) << setiosflags(ios::fixed) << "4" << " \t " << "1" << " \t " << e_particle_id << " \t " << "1" << " \t " << "0" <<  " \t "<< "0" <<  " \t " << e_momx_lab << " \t " << e_momy_lab << " \t " << e_momz_lab << " \t " << e_energy_lab << " \t " << emass << " \t " << vex_Lab  << " \t " << vey_Lab << " \t " << vez_Lab << endl; 
        // final state neutron (TDIS)
    OUT_pythia << setiosflags(ios::left) << setiosflags(ios::fixed) << "5" << " \t " << "1" << " \t " << sp_particle_id << " \t " << "2" << " \t " << "0" <<  " \t "<< "0" <<  " \t " << n_momx_lab << " \t " << n_momy_lab << " \t " << n_momz_lab << " \t " << n_energy_lab << " \t " << spmass << " \t " << vnx_Lab  << " \t " << vny_Lab << " \t " << vnz_Lab << endl;  
    // final state pion
    OUT_pythia << setiosflags(ios::left) << setiosflags(ios::fixed) << "6" << " \t " << "1" << " \t " << pi_particle_id << " \t " << "2" << " \t " << "0" <<  " \t "<< "0" <<  " \t " << pi_momx_lab << " \t " << pi_momy_lab << " \t " << pi_momz_lab << " \t " << pi_energy_lab << " \t " << pimass << " \t " << vpix_Lab  << " \t " << vpiy_Lab << " \t " << vpiz_Lab << endl; 
    ...
    // final state neutron (TDIS)
    OUT_hepmc << setiosflags(ios::left) << setiosflags(ios::fixed) << "P" << " " << "4"  << " " << "-1" << " " << sp_particle_id << " " << n_momx_lab << " " << n_momy_lab << " " << n_momz_lab << " " << n_energy_lab << " " << spmass <<  " " << "1" << endl;    
    // final state pion
    OUT_hepmc << setiosflags(ios::left) << setiosflags(ios::fixed) << "P" << " " << "5"  << " " << "-1" << " " << pi_particle_id << " " << pi_momx_lab << " " << pi_momy_lab << " " << pi_momz_lab << " " << pi_energy_lab << " " << pimass <<  " " << "1" << endl;
